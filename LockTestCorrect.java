...
public class LockTestCorrect {
    public static void main(String[] args) {
        ...
        // Thread for updating an employee with a delay
        Runnable updateTask = () -> {
            System.out.println("Attempting to update employee...");
            try {
                db.getLock().writeLock().lock();
                try {
                    Thread.sleep(2000);  // Holding the lock for 2 seconds
        ...
        Runnable readTask = () -> {
            ...
            try {
                boolean lockAcquired = db.getLock().readLock().tryLock(3, TimeUnit.SECONDS);
                if (lockAcquired) {
                    try {
